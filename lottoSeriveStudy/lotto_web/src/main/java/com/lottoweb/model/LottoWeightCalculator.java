...
public class LottoWeightCalculator {
	/**
	 * 과거 당첨 번호 리스트로부터 1~45 숫자에 대한 가중치를 계산한다.
	 *
	 * 입력 리스트가 비어있으면 균등 가중치(모두 1.0)를 반환한다.
	 * 가중치는 (빈도 기반 50%) + (정규분포 기반 50%)의 가중 평균으로 계산되며,
	 * 마지막에 합이 1이 되도록 정규화한다. 합이 0/NaN이면 균등 가중치로 대체한다.
	 */
    public static Map<Integer, Double> calculateWeights(List<Integer> allNumbers) {
		...
		// 1) 빈도수 계산: 과거 데이터에서 각 숫자가 등장한 빈도
		//    존재하지 않는 숫자는 0으로 초기화된다.
		...
		// 2) 정규분포 기반 가중치: 평균에 가까운 수로 높은 가중치
		//    표준편차가 0이거나 음수인 경우 균등 가중치로 대체한다.
		...
		// 4) 정규화: 합이 1이 되도록 스케일링 (합이 0/NaN이면 균등 가중치로 대체)
	...
	/**
	 * 가중치 맵을 이용해 k개의 번호를 비복원 추첨한다.
	 *
	 * 각 반복마다 누적 가중치로 가중 선택을 수행하고,
	 * 선택된 번호의 가중치를 정규화하여(totalWeight에서) 제거한다.
	 * 보너스 번호는 1~45 범위에서 중복 없이 균등 랜덤으로 선택한다.
	 */
    public static Map<String, Object> drawLotto(Map<Integer, Double> weights, int k) {
		// 번호 리스트와 해당 가중치 리스트 동기화 (인덱스 동기화 보장)
		...
		for (int i = 0; i < k; i++) {
			// 0 ~ totalWeight 사이에서 균등 랜덤 값을 생성
		...
		// 보너스 번호 선택: 이미 선택된 번호는 제외하여 중복 방지
